...
export abstract class WidgetBase implements IWidget, OnDestroy, OnInit, AfterViewInit {
    ...
    protected constructor(protected _widgetInstanceService: WidgetInstanceService,
        ...
        this._optionsService.listenForGlobalOptionsChanges().subscribe(options => {
             ...
             * The following method needs to also be called when the widget is initially instantiated.
             * When the widget is instantiated the option values need to come from the persistent store.
             */

            this.updateWidgetWithGlobalOptions(options);
        });

        this.updateWidgetWithGlobalOptions(this._optionsService.getBoardOptions())
    }
    ...
    public showWidgetControls(enable: boolean) {
        this.showControls = true; //this is the place to set editmode or read mode
    }
     ...
     * */
    public configureWidget(instanceId: number, config: any, tags: Array<any>) {
    ...
    public updateWidgetWithGlobalOptions(options: any) {
